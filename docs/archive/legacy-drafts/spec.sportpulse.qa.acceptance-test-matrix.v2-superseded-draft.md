...
- I-01
- J-01, J-02
- K-01, K-02, K-03, K-04, K-05, K-06, K-07

K-08 is not required to claim MVP completion by default. It becomes required before claiming full active-product global style propagation readiness.
...
**Precondition (free tier):** anonymous or authenticated free user with active commercial display ad slot configured.
**Expected:** commercial display ad slot may render according to placement policy.
**Precondition (Pro tier):** authenticated user with active Pro subscription and same commercial display ad slot configured.
**Expected:** commercial display ad slot does not render.
**Invariant:** operational notices, degraded-state warnings, service-status banners, and mandatory product notices remain visible to all tiers.
**Pass gate:** free user may see commercial display ad slot; Pro user DOM contains no commercial display ad output; operational/system notices still render when active.

### K-08 — Global theme propagation readiness
**Precondition:** active product surface inventory defined; at least two approved theme packs available (for example default + seasonal or light/dark mapped variants); Level B style rollout declared by implementation.
**Expected:** shell, primary navigation, competition controls, dashboard containers, treemap surrounds, detail panel, match cards, prediction blocks, track record surfaces, paywall surfaces, auth/session surfaces, Pro page surfaces, and loading/empty/error/degraded states render through semantic tokens or documented exceptions.
**Invariant:** theme swap must not require per-surface manual patching in active product surfaces; warnings, operational notices, editorial/commercial announcements, paywall, and free-vs-Pro surfaces retain distinct semantics under theme change.
**Pass gate:** theme swap changes the active product footprint without undocumented raw visual leakage; any remaining exception is explicitly documented and bounded; semantic distinctions remain intact.
...
This acceptance matrix defines the concrete pass/fail conditions for SportPulse across canonical normalization, signal computation, scoring policy execution, deterministic treemap geometry, snapshot assembly, UI API contracts, degraded-state handling, frontend rendering, regression/version discipline (A–J), and the prediction surface plus track record (K). The product is considered real only when deterministic, explainable, snapshot-first behavior is verified under both normal and degraded conditions, prediction probabilities are correctly gated by operating mode and subscription tier, track record data is accurate and cherry-pick-proof, Pro ad suppression behaves correctly, and any claim of full product-wide style propagation is backed by explicit readiness validation rather than assumption.
